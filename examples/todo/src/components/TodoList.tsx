import type {TodoList_user$key} from '../../__generated__/relay/TodoList_user.graphql';
...
export default function TodoList({
  ...
  const user = useFragment<TodoList_user$key>(graphql`
  ...
  const commitMarkAllTodosMutation = useMarkAllTodosMutation(user, user.todos.edges);
  ...
  return <>
    <header className="header">
      ...
      <TodoTextInput className="new-todo" onSave={handleOnSave} placeholder="What needs to be done?" />
    </header>

    <section className="main">
      <input checked={user.totalCount === user.completedCount} className="toggle-all" onChange={handleMarkAllChange}
             type="checkbox" />

      <label htmlFor="toggle-all">Mark all as complete</label>

      <ul className="todo-list">
        {user.todos.edges.map(({
                                 node,
                               }) => <Todo key={node.id} todoRef={node} userRef={user}
